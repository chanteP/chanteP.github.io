define('spal', function(require, exports){
	var defaultPage = 'home', pageCache = {}, pageWrap = $.find('#wrapper'), pageModulePrefix = 'page/';
	...
	var page = {
		...
		'init' : function(pageName){
			...
			if(pageName && !pageCache[pageName]){
				pageCache[pageName] = $.create(template['404']);
			}
			page.clear();
		},
		...
		'load' : function(pageName){
			...
			iframe.onload = function(e){
				pageCache[pageName] = pageCache[pageName] || $.create(template['404']);
		...
		'clear' : function(){
			[].forEach.call($.findAll('[id=spalScript]'), function(script){
				$.remove(script);
			});
		}
	}
